...
class FruitsManager:
    ...
    def OutData(self):
        ...
        print(self.fruitslist.loc[self.fruitslist['fruitname']==outfruitname, 'count'])
            while  True: 
                ...
                try:
                    ...
                    if outcount <= int(self.fruitslist.loc[self.fruitslist['fruitname']==outfruitname, 'count']):
                        pass
                    else:
                ...
                for i in self.fruitslist.index:
                    while self.fruitslist['fruitname'][i] == outfruitname:
                        self.fruitslist.loc[self.fruitslist['fruitname']==outfruitname, 'count'] = int(self.fruitslist.loc[self.fruitslist['fruitname']==outfruitname, 'count']) - outcount
                        if int(self.fruitslist.loc[self.fruitslist['fruitname']==outfruitname, 'count'])==0:
                            self.fruitslist.drop(np.where(self.fruitslist['fruitname'] == outfruitname),inplace=True)
                        break
